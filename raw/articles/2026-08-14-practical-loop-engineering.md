...
tags: [loop-engineering, goal, loop, scheduled-tasks, routines, verification, maker-checker, subagents, worktrees, pr-triage, claude-code, delegation]
---

# Practical Loop Engineering

> Goals, loops, and the discipline of not delegating your judgment. `/goal`은 검증 가능한 정지 조건까지 밀어붙이는 primitive, `/loop`는 주기로 재실행하는 스케줄러. Claude Code 팀이 분류한 네 종류의 루프와, 저자가 매일 5~10개 에이전트를 돌리며 무엇을 위임하고 무엇을 지켜보는지.
